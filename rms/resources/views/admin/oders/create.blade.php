...
const menuItems = @json($menuItems->pluck('price', 'id'));
...
function addItem() {
    ...
    div.innerHTML = `
        ...
        <div class="col-md-3">
            <label class="form-label small">Note</label>
            <input type="text" name="items[${itemIndex}][note]"
                   class="form-control" placeholder="e.g. no onion">
        </div>
        <div class="col-md-2">
...
function getMenuOptions() {
    ...
    @foreach($items as $item)
    opts += '<option value="{{ $item->id }}" data-price="{{ $item->price }}">{{ $item->name }} — Rs.{{ number_format($item->price, 0) }}</option>';
    @endforeach
    opts += '</optgroup>';
    @endforeach
    return opts;
...
function calculateTotal() {
    ...
    document.querySelectorAll('.order-item').forEach(row => {
        ...
        if (select && select.value && qty) {
            const price = menuItems[select.value] || 0;
